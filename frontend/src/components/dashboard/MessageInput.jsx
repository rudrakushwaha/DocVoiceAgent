...
export default function MessageInput({onSend, onStartVoice, onStopVoice, isRecording, recordingTime, voiceOutput, setVoiceOutput, isProcessingVoice}){
  ...
  return (
    <div className="input-area">
      ...
      <button className="icon-btn" title={isRecording ? "Stop Recording" : "Voice"} onClick={isRecording ? onStopVoice : onStartVoice} style={isRecording ? {background:'var(--accent-color)',color:'#fff',boxShadow:'0 0 8px var(--accent-color)'} : {}}>
        {isRecording ? (
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><circle cx="12" cy="12" r="8" fill="#fff" stroke="var(--accent-color)" strokeWidth="2"/></svg>
        ) : (
          <svg width="18" height="18" viewBox="0 0 24 24" fill="none"><path d="M12 1v11" stroke="var(--text-secondary)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/><path d="M19 11a7 7 0 01-14 0" stroke="var(--text-secondary)" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round"/></svg>
        )}
      </button>
      <div style={{display:'flex',flexDirection:'column',gap:8}}>
        {isRecording && <div style={{color:'var(--accent-color)',fontWeight:500}}>Recording... {recordingTime}s</div>}
        {isProcessingVoice && <div style={{color:'var(--accent-color)',fontWeight:500}}>Processing voice…</div>}
